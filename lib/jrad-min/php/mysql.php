...
function sql_update($mysqli, $tb, $post_array, $field, $value)
{
    ...
    // execute sql command
    if ($field == '*' && $value == 1) {
        $sql_stmt = 'UPDATE `' . $tb . '` SET ' . $hash_map . ' WHERE 1';
    } else {
...
function sql_update_all($mysqli, $tb, $post_array)
{
    return sql_update($mysqli, $tb, $post_array, '*', 1);
}

function sql_update_id($mysqli, $tb, $post_array, $id)
{
    return sql_update($mysqli, $tb, $post_array, 'id', $id);
}
...
function sql_delete_id($mysqli, $tb, $id)
{
    return sql_delete($mysqli, $tb, 'id', $value);
}
...
function sql_fetch_by_week($mysqli, $tb, $where = 'DATE', $wk = null, $yr = null)
{
    $wk = is_null($wk) ? date('W') : $wk;
    $yr = is_null($yr) ? date('Y') : $yr;
    $sql_stmt = 'SELECT * FROM `' . $tb . '` WHERE WEEK(' . $where . ')="' . $wk . '" AND YEAR(' . $where . ')="' . $yr . '" ORDER BY id DESC';
    $result = sql_query($mysqli, $sql_stmt);
    return $result;
}
