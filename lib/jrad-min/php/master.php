...
function sterilize_request($post_array, $post_array_keys)
{
    $new_post_array = array();
    foreach ($post_array as $key => $value) {
        if (!is_array($value) && strtolower($key) != 'id' && in_array($key, $post_array_keys)) {
            $value = trim($value);
            $value = stripslashes($value);
            $value = htmlspecialchars($value);
            $new_post_array[$key] = $value;
...
function change_password($post_array, $psw_old)
{
    $p = array_keys($post_array);
    $psw_cur = $post_array[$p[0]];
    $psw_new = $post_array[$p[1]];
    $psw_cfm = $post_array[$p[2]];

    #var_dump([$psw_old, $psw_cur, $psw_new, $psw_cfm]);
    if ($psw_cur == $psw_old) {
        if ($psw_cur == $psw_new) {
            $res = 'Current password and new password cannot be the same';
        } else if ($psw_new != $psw_cfm) {
            $res = 'New password and confirm password does not match';
        } else {
            $res = array('password' => $psw_new);
        }
    } else {
...
function upload_file_seed()
{
    return array(
        'name' => 'manifest.json',
        'type' => 'application/json',
        'tmp_name' => 'C:\wamp64\tmp\phpE5B.tmp',
        'error' => 0,
        'size' => 535,
    );
}

function upload_file($file_array, $dir, $use_file_name = false)
{
    $ext = '.' . explode('.', $file_array['name'])[1];
    if ($use_file_name === true) {
        $new_file_name = $file_array['name'];
    } else if ($use_file_name === false) {
        $mktime = ('YmdHisu');
        $new_file_name = $mktime . $ext;
    } else {
        $new_file_name = strpos($use_file_name, '.') > 0 ? $use_file_name : $use_file_name . $ext;
    }
...
function upload_files($file_assoc, $dir, $use_file_name = false)
{
    $res = array();
    foreach ($file_assoc['name'] as $i => $j) {
        $file_array = array(
            'name' => $file_assoc['name'][$i],
            'type' => $file_assoc['type'][$i],
            'tmp_name' => $file_assoc['tmp_name'][$i],
            'error' => $file_assoc['error'][$i],
            'size' => $file_assoc['size'][$i],
        );
        $res[$j] = upload_file($file_array, $dir, $use_file_name);
    }
    return $res;
}
...
function money_f($amt)
{
    return is_numeric($amt) && $amt > 0 ? number_format((int) $amt) : 0;
}
...
function mailto_f($email = 'webmaster@hwplabs.com')
{
    if (strpos($email, '@') > -1) {
        $email = strtolower($email);
        return '<a href="mailto:' . $email . '" target="_new">' . $email . '</a>';
    }
    return $email;
}

function tel_f($phone = '+2348169960927')
{
    if (strpos($phone, '+') > -1) {
        return '<a href="tel:' . $phone . '" target="_new">' . $phone . '</a>';
    }
    return $phone;
}

function wa_f($phone = '2348169960927')
{
    if (is_numeric($phone)) {
        return '<a href="https://wa.me/' . $phone . '" target="_new">' . $phone . '</a>';
    }
    return $phone;
}
